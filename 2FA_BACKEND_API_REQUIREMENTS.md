...
- If user has only email enabled: `twoFactorMethod: "email"`
- If user has only TOTP enabled: `twoFactorMethod: "totp"`
- If user has **both enabled**: `twoFactorMethod: "both"` or default to one (e.g., "email") and include `availableMethods: ["email", "totp"]`
- When both are enabled, the frontend can allow user to choose, or default to one with option to switch
- Email code should be sent immediately if email is one of the available methods
...
- If user has **both methods enabled**:
  ...
  - Send email code immediately (user might choose email)
...
- **For users with both methods enabled**: This only resends the email code, TOTP codes are generated by the app and don't need resending
